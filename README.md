...
> **Le scouting report complet pour analyser l'adversaire et préparer le match.**
> Repo réservé — le développement n'a pas encore commencé.

## Stack prévue (standard de la galaxie)

- **Next.js** (App Router) déployé sur **Vercel** — sous-domaine `scouting.ebok.fr`
- **Clerk** pour le compte unique EBOK (voir `docs/AUTH.md` du repo
  [EBOK-BASKETBALL](https://github.com/marleyebok/EBOK-BASKETBALL))
- **Neon Postgres**, schéma `scouting` + référence à la table partagée `shared.users`
